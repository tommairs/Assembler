...
$segmented = $_POST["segmented"];
$filteredcount = $_POST["filteredcount"];
...
{
	global $key, $template, $recipients, $now, $date, $hour, $minutes, $tz, $filteredcount, $campaign, $returnpath, $open, $click, $email; 
	...
	if (strlen($jsonLoad) > 2) $transmissionLoad .= '"description" : "' . $filteredcount . ' recipients targeted",';
	else $transmissionLoad .= '"description" : "' . $recipientCount . ' recipients targeted",';
	...
	if (($recipients == "cgJson") or (strlen($jsonLoad)>2))
	{
...
function sendConfirmationEmail($validationResponse, $response)
{
	...
	global $key, $email, $campaign, $apiroot, $returnpath;
	global $meta1, $data1, $meta2, $data2, $meta3, $data3, $meta4, $data4, $meta5, $data5;
	$singlequoteResponse = str_replace('"',"'",$response);
	$singlequoteResponse = str_replace(array('{', '}'), array(""),$singlequoteResponse);
	//Build the payload
	$emailReceipt = '{"content":{"from": {"name": "SparkPost Campaign Admin","email": "' . $returnpath . '"},';
	$emailReceipt .= '"subject" : "Your Campaign receipt for ' . $campaign . '", ';
	...
	$emailReceipt .= '"return_path" : "' . $returnpath . '", ';
	$emailReceipt .= '"campaign_name": "' . $campaign . '",';
        if (($meta1 != "") or ($meta2 != "") or ($meta3 != "") or ($meta4 != "") or ($meta5 != ""))
        {
                $emailReceipt .= '"metadata" : {';
                if ($meta1 != "") {$emailReceipt .= '"' . $meta1 . '":"' . $data1 . '",';}
                if ($meta2 != "") {$emailReceipt .= '"' . $meta2 . '":"' . $data2 . '",';}
                if ($meta3 != "") {$emailReceipt .= '"' . $meta3 . '":"' . $data3 . '",';}
                if ($meta4 != "") {$emailReceipt .= '"' . $meta4 . '":"' . $data4 . '",';}
                if ($meta5 != "") {$emailReceipt .= '"' . $meta5 . '":"' . $data5 . '"';}
                $emailReceipt = trim($emailReceipt, ",");
                $emailReceipt .= "}";
        }
	$emailReceipt .= "}";
