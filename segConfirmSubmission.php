...
<ul class="topnav" id="generatorTopNav">
  ...
  <li><a class="active" href="segEntryPage.php<?php echo '?apikey=' . $hash .'&apiroot=' . $apiroot ?>">Generate Campaign</a></li>
...
//
if (empty($email)) $email = "Not Entered";
...
else
{
	if ($segmented == "TRUE")
	{
		$validationText .= "<h5>Recipients were filtered/segmented from recipient list: '" . $recipients . "' from a size of " . $recipientCount . " recipients to a segmented size of " . $filteredcount . "</h5>";
	}
	else
	{
...
function confirmedSend() 
{
    ...
    var segmented = "<?php echo $segmented; ?>";
    var filteredcount = "<?php echo $filteredcount; ?>";
    
    $.ajax({
      
    url:'segSendCampaign.php',
    type: "POST",
    data: {"apikey" : apikey, "apiroot" : apiroot, "template" : template, "recipients" : recipients, "validationText" : validationText, "email" : email, "jsonLoad" : jsonLoad, "recipientCount" : recipientCount, "filteredcount" : filteredcount,
      	   "segmented" : segmented, "campaign" : campaign, "returnpath" : returnpath, "open" : open, "click" : click, "now" : now, "date" : date, "hour" : hour, "minutes" : minutes, "tz" : tz, "globalsub" : globalsub,
