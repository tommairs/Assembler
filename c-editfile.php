...
// Check to see if a Transmission Payload exists already
  $tmpfilecontents = file_get_contents($filedir ."/payload.json");

// If not, create a skeleton
...
// replace the appropriate sections of the payload 
//with sanitization and validation
  $tmpdata = preg_replace("/\"/","\\\"",$tmpdata);
//check for header row
// load data into array
// validate that "email" exists
  // post error message if no email address and exit
// itemize additional fields
// convert to JSON
// update tempalte
...
$recipients = "{ "address": {"email": "placeholder@here.com","name": "placeholder"},
              "substitution_data": {"sender":"placeholder"}}";

  $tmpfilecontents = preg_replace("/\"recipients\": \[.*\]/","\"recipients\": \[". $recipients ."\]",$tmpfilecontents);
